...
usage() {
  echo "Usage: $0 [--code-review-graph] [--arxiv] [--searxng-mcp] [--hermes] [--all]"
  echo ""
  ...
  echo ""
  echo "code-review-graph: uv tool install + registers MCP (merges with your config)."
}
...
do_hermes() {
  ...
  if [[ "${SKIP_HERMES_GATEWAY_RESTART:-0}" != "1" ]]; then
    bash "${ROOT}/scripts/restart-hermes-gateway.sh"
  fi
