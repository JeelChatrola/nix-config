...
SKIP_HERMES_GATEWAY_RESTART=1 bash ai-stack/scripts/install-optional-agents.sh --code-review-graph --arxiv --searxng-mcp --hermes
echo ""

if bash ai-stack/scripts/ai-stack-docker-wanted.sh; then
  echo "Starting Docker stack (Ollama, SearXNG)..."
  bash "$AI_STACK_DIR/scripts/docker-compose.sh" up -d
  echo ""
  ...
  echo ""
  echo "Models:        edit ai-stack/stack-models.json; ollama.pull[]"
  echo "Pull ad-hoc:   ollama-pull <tag>"
else
  echo "Skipping Docker stack (set AI_STACK_DOCKER=0 in ai-stack/.env, or use --no-docker)."
  echo "OpenCode uses symlinks to ai-stack/generated/*.json; start containers with: ai-up"
  echo ""
...
bash "$AI_STACK_DIR/scripts/restart-hermes-gateway.sh"
echo ""

echo "CLI agents:    opencode, hermes  (hermes setup once for ~/.hermes/)"
echo ""
